import React from 'react';
import styled from '@emotion/styled';

export const WaveDown = ({ color = '#fff' }) => (
  <SvgDown viewBox="0 0 580 150">
    <path
      d="m-0.5,-1.5625l0.5,62.5625c0.5,0.4375 30.5,20.4375 54.5,14.4375c24,-6 12,-34 55,-18c43,16 96,53 172,29c76,-24 118,-50 181,-46c63,4 117,44 116.5,43.5625c0.5,0.4375 1.5,-84.5625 1,-84l-580.5,-1.5625z"
      fill={color}
    />
  </SvgDown>
);

export const WaveUp = ({ color = '#fff' }) => (
  <SvgUp viewBox="0 0 580 150">
    <g>
      <path d="m-0.74337,0.32227" fill={color} />
      <path d="m-0.00262,-0.23216l150.37435,85.41812" fill={color} />
      <path d="m580.00262,149.39882" fill={color} />
      <path
        d="m0.39738,-0.16254c2.50263,1.41735 22.96317,79.58383 87.4082,76.60627c64.44502,-2.97757 77.77848,-55.8293 137.77902,-40.94149c60.00054,14.88781 64.44503,55.08492 118.51959,50.61857c54.07456,-4.46634 102.22315,-40.94149 148.14949,-36.47514c45.92634,4.46634 86.67268,53.31834 87.41081,53.82943c0.73813,0.51109 0.00262,45.64114 0.74075,46.15223c0.73813,0.51109 -580.00262,0.23331 -580.00524,0.74439l-0.00262,-150.37172z"
        fill={color}
      />
    </g>
  </SvgUp>
);

const SvgDown = styled.svg`
  position: absolute;
  top: -10px;
  left: 0;
  right: 0;
`;

const SvgUp = styled.svg`
  position: absolute;
  bottom: -10px;
  left: 0;
  right: 0;
`;
